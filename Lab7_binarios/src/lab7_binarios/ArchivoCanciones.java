...
public class ArchivoCanciones {
    ...
    public Cancion[] leerTodos() throws IOException {
        try(RandomAccessFile raf=new RandomAccessFile(NOMBRE_ARCHIVO,"r")) {
            ...
            Cancion[] resultado=new Cancion[count];
    ...
    public void actualizarRutaMP3(int id,String nuevaRuta) throws IOException {
        try(RandomAccessFile raf=new RandomAccessFile(NOMBRE_ARCHIVO,"rw")) {
            int total=(int)(raf.length()/TAM_REGISTRO);
            if(id<0||id>=total) return;
            long offsetRutaMP3=((long)id*TAM_REGISTRO)+4+(TAM_NOMBRE*2)+(TAM_ARTISTA*2)+(TAM_GENERO*2)+4;
            raf.seek(offsetRutaMP3);
            escribirString(raf,nuevaRuta,TAM_RUTA);
        }
    }
